...
export default function ProductCard({ product, onQuickView }) {
  ...
  return (
    ...
    >
      ...
      {/* Product Name (Fixed 2 lines max with uniform height so cards align) */}
      <h3 className="text-xs font-medium text-stone-800 line-clamp-2 leading-snug group-hover:text-[#D61C7C] transition-colors mb-1.5 h-[32px]">
        {product.name}
      </h3>
      ...
      {/* Add to Cart button (sits directly below rating with zero excess void) */}
      <button
        onClick={handleAddToCart}
        className={`w-full mt-auto py-2 px-2.5 rounded-xl text-xs font-semibold flex items-center justify-center gap-1.5 transition-all duration-200 shadow-xs cursor-pointer ${
          isAdded
            ? 'bg-emerald-600 text-white'
            : 'bg-[#D61C7C] hover:bg-[#BF156C] text-white hover:shadow-xs'
        }`}
      >
        {isAdded ? (
          <>
